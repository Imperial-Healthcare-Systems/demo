...
/**
 * Prebuild the articles that exist at build time.
 *
 * `dynamicParams` is left at its default of true on purpose, and it is what
 * makes the portal work: an article the client publishes tomorrow has no entry
 * here, and with `dynamicParams: false` its URL would 404 until the next
 * deploy. Left true, the first request for a new slug renders it and caches
 * the result — so a post published from the portal is reachable immediately.
 */
export async function generateStaticParams() {
  return (await getAllInsights()).map((insight) => ({ slug: insight.slug }));
}
              ...
              <div className="flex flex-col gap-6 lg:col-span-7">
                ...
                */}
                <Link
                  href="/insights"
                  scroll={false}
                  className="group/back -mt-2 inline-flex min-h-11 w-fit items-center gap-2 text-[0.8125rem] font-medium text-ink-2 transition-colors hover:text-navy-600 md:min-h-0"
                >
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  <span className="rounded-full bg-white px-3 py-1 font-mono text-[0.75rem] md:text-[0.625rem] uppercase tracking-[0.14em] text-ink-2 ring-1 ring-line">
                    {insight.type}
                  </span>
                  <span className="rounded-full bg-white px-3 py-1 font-mono text-[0.75rem] md:text-[0.625rem] uppercase tracking-[0.14em] text-ink-2 ring-1 ring-line">
                    {insight.topic}
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-line pt-6">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-navy-600 font-display text-[0.75rem] md:text-[0.6875rem] font-bold text-white">
                      {insight.author
                        .split(/\s+/)
                        .map((word) => word[0])
                        .join("")
      ...
      <DarkSection grid={false}>
        <div className="shell section">
          <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link
              href="/insights"
              className="group/all inline-flex min-h-11 items-center gap-2 text-sm font-medium text-sky-400 hover:text-white md:min-h-0"
            >
              View all
              <Icon
                name="arrowRight"
