              ...
              return (
                ...
                >
                  ...
                  */}
                    ...
                    <p
                      className={cn(
                        "mt-5 font-mono text-[0.75rem] md:text-[0.6875rem] tracking-[0.16em] uppercase",
                        tone.label,
                      )}
                    >
                      {d.tag}
              ...
              return (
                ...
                >
                  <FlipCard
                    ...
                    front={
                      <>
                        ...
                        */}
                        ...
                        <p className="mt-4 font-mono text-[0.75rem] md:text-[0.625rem] tracking-[0.16em] text-navy-600 uppercase">
                          {domain.tag}
                        </p>
                    ...
                    back={
                      <>
                        <p className="font-mono text-[0.75rem] md:text-[0.625rem] tracking-[0.18em] text-sky-400 uppercase">
                          {domain.title}
                        </p>
                        <ul className="mt-3.5 flex flex-wrap gap-1.5">
                          {domain.tags.map((tag) => (
                            <li
                              key={tag}
                              className="rounded-full px-2.5 py-1 font-mono text-[0.75rem] md:text-[0.625rem] tracking-[0.1em] text-ink-inv-2 uppercase ring-1 ring-white/20"
                            >
                              {tag}
                            </li>
                          ))}
      ...
      <DarkSection id="delivery-lifecycle">
        <div className="shell section">
          ...
          <ol className="relative mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <span
              aria-hidden="true"
              className="pointer-events-none absolute top-[3.25rem] right-0 left-0 hidden h-px bg-[linear-gradient(90deg,transparent,rgba(255,255,255,.22)_12%,rgba(255,255,255,.22)_88%,transparent)] lg:block"
            />

            {/*
              Arrowheads on the rule, one per gap.

              The line alone said "these four are related". It did not say which
              way to read them, which is the whole point of a lifecycle — and on
              a page where every other row of four cards is a set rather than a
              sequence, a plain rule is not enough to tell them apart.

              The offsets are exact rather than eyeballed. With four equal
              columns and a 1.5rem gap, the gap centres land at 25% - g/4, 50%
              and 75% + g/4 — so 0.375rem either side of the quarter marks. A
              flat 25/50/75 would sit 6px off centre in the outer two, which is
              visible once there is a mark there to notice.
            */}
            {["calc(25% - 0.375rem)", "50%", "calc(75% + 0.375rem)"].map((left) => (
              <span
                key={left}
                aria-hidden="true"
                style={{ left }}
                className="pointer-events-none absolute top-[3.25rem] hidden h-6 w-6 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-abyss-2 text-sky-400 ring-1 ring-white/15 lg:flex"
              >
            ...
            {deliveryLifecycle.steps.map((step, i) => (
              <Reveal
                ...
                key={step.num}
                delay={i * 90}
                className="relative flex flex-col rounded-[var(--radius-card)] bg-white/[0.14] p-6 ring-1 ring-white/20 transition-[background-color,transform] duration-300 hover:-translate-y-0.5 hover:bg-white/[0.2] md:p-7"
              >
                <p className="font-mono text-[0.75rem] md:text-[0.6875rem] tracking-[0.16em] text-sky-400 uppercase">
                  {step.num} / {step.phase}
                </p>
