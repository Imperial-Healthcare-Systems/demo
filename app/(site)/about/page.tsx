...
export default function AboutPage() {
  /*
    One entry or two. The vision always renders; the mission joins it when the
    client supplies one, and the layout switches from a centred single
    statement to a two-up on its own.
  */
  const statements = [aboutPage.vision, aboutPage.mission].filter(
    ...
    <>
            ...
            >
              ...
              */}
              ...
              <div className="-mt-8 flex flex-col gap-3">
                ...
                <p className="font-mono text-[0.75rem] md:text-[0.6875rem] uppercase tracking-[0.2em] text-navy-600">
      ...
      */}
        ...
        <div className="shell">
          ...
          <div className="grid gap-x-14 gap-y-12 md:grid-cols-2">
            {aboutPage.story.map((block, i) => (
              ...
              >
                <span className="font-mono text-[0.75rem] md:text-[0.6875rem] tabular text-navy-600">
      ...
      {/*
        ...
        Given a band of its own at the client's request, and everything about
        it is doing one job: make this read as a statement rather than as more
        page. The ground goes dark, which nothing either side of it does, so
        the section announces itself before a word is read. The type steps up
        to display size and the sentence is centred and held to about 20 words
        a line — the measure a statement wants and body copy does not. Two soft
        radials sit behind it so the panel has depth instead of being a flat
        navy rectangle, and the mark above it is drawn small: the eye should
        land on the words.

        `mission` renders beside it automatically the moment the client
        supplies one — see the note in content/about.ts for why it is not
        written yet.
      */}
      <DarkSection className="py-24 md:py-32">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(70%_100%_at_50%_0%,rgba(1,164,255,.20),transparent_70%),radial-gradient(50%_80%_at_85%_100%,rgba(1,172,50,.16),transparent_70%)]"
        />
        <div className="shell">
          <div
            className={
          ...
          >
            {statements.map((entry) => {
              /*
                Split rather than stored twice. The accent has to be a literal
                slice of the statement, so if it ever stops matching the whole
                sentence still prints — just without the highlight. A missing
                gradient is a smaller failure than a missing half-sentence.
              */
              const at = entry.statement.indexOf(entry.accent);
              const before = at >= 0 ? entry.statement.slice(0, at) : entry.statement;
              const after = at >= 0 ? entry.statement.slice(at + entry.accent.length) : "";
              ...
              return (
                ...
                >
                  <Eyebrow onDark>{entry.label}</Eyebrow>
                  <p className="text-[1.5rem] leading-[1.28] font-semibold tracking-[-0.03em] text-white text-balance md:text-[2rem] lg:text-[2.375rem]">
                    {before}
                    {at >= 0 && (
                      <span className="text-brand-gradient-inv">{entry.accent}</span>
                    )}
                    {after}
                  </p>
