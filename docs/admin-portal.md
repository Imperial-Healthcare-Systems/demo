...
- **Page views** — every page opened, including repeat visits.
- **Visitors** — individual people per day.
...
network address and browser, mixed with a salt that changes at midnight UTC —
so nobody can be identified and nobody can be followed from one day to the next.
That is why the site needs no tracking consent banner.

The honest caveat: because the salt rotates daily, one person visiting on three
days counts as three in a 30-day visitor figure. Today's number is exact.
Obvious crawlers are excluded, and views of the portal itself are not counted.
...
| `app/api/track/` | the public endpoint that records an event |
